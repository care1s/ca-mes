  ...
  <div class="app-container">
    ...
    <div class="page-header">
      <div class="title-section">
        ...
        <span class="subtitle">Vendor Management</span>
      </div>
    ...
    <el-card class="search-card" shadow="never">
      <el-form :inline="true" :model="queryParams" class="search-form">
        <el-form-item label="编码">
          <el-input v-model="queryParams.vendorCode" placeholder="请输入编码" clearable />
        </el-form-item>
        <el-form-item label="名称">
          <el-input v-model="queryParams.vendorName" placeholder="请输入名称" clearable />
        </el-form-item>
        <el-form-item>
    ...
    <el-card class="table-card" shadow="never">
      ...
      >
        ...
        <el-table-column prop="vendorCode" label="编码" width="120" show-overflow-tooltip />
        <el-table-column prop="vendorName" label="名称" min-width="150" show-overflow-tooltip />
...
<script>
import { listVendor, addVendor, updateVendor, delVendor, delVendorBatch } from '@/api/md'
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        vendorCode: [{ required: true, message: '请输入供应商编码', trigger: 'blur' }],
  ...
  methods: {
    ...
    handleView(row) {
      this.$alert(`编码：${row.vendorCode}<br>名称：${row.vendorName}<br>联系人：${row.contactPerson || '-'}<br>电话：${row.phone || '-'}`, '供应商详情', {
    ...
    handleDelete(row) {
      this.$confirm(`确定删除供应商 "${row.vendorName}" 吗？`, '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(async () => {
        await delVendor(row.vendorId)
    ...
    async handleStatusChange(row) {
      ...
      try {
        await updateVendor({ vendorId: row.vendorId, status: row.status })
        ...
        if (valid) {
          try {
            if (this.form.vendorId) {
              await updateVendor(this.form)
              this.$message.success('更新成功')
            } else {
              await addVendor(this.form)
